...
type ShellProps = { children: ReactNode; active?: ActiveMenu };
...
export function Shell({ children, active }: ShellProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("bomcare:ui:railPinned:v2", railPinned ? "true" : "false");
  }, [railPinned]);
  ...
  const menus = useMemo(
    () => [
      ...
      { key: "approvals" as MenuKey, href: "/approvals", label: "전자결재", activeKey: "approvals" as ActiveMenu },
      { key: "staff" as MenuKey, href: "/staff", label: "직원관리", activeKey: "staff" as ActiveMenu },
  ...
  const visibleMenus = useMemo(() => (role ? menus.filter((m) => canAccess(role, m.key)) : []), [menus, role]);
  const logout = () => { clearSessionUser(); router.push("/login"); };
      ...
      <main className="landing">
        <section className="landing-hero">
          <div className="landing-copy">
            ...
            <h1>로그인 후 서비스 화면으로 이동할 수 있습니다.</h1>
            <p>권한 기반 메뉴(RBAC)가 적용되어 계정 역할에 따라 접근 가능한 기능이 달라집니다.</p>
  ...
  return (
    <div className={`app-shell ${railOpen ? "rail-expanded" : "rail-collapsed"}`}>
      <aside className="rail" onMouseEnter={() => setRailHover(true)} onMouseLeave={() => setRailHover(false)}>
        <button type="button" className="rail-toggle" onClick={() => setRailPinned((v) => !v)}>{railPinned ? "고정됨" : "메뉴"}</button>
        {!railOpen ? (
          <nav className="rail-mini-nav">
            {visibleMenus.slice(0, 9).map((menu) => (
              <Link key={`mini-${menu.href}`} href={menu.href} className={pathname === menu.href ? "active" : ""} title={menu.label}>
        ...
        <div className="rail-main">
          <div>
            ...
            <h1>늘봄 아동복지 운영센터</h1>
            <p className="rail-copy">사례관리, 생활지원, 행정문서, 결재 흐름을 통합한 아동 사회복지시설 운영 플랫폼입니다.</p>
          ...
          <nav className="rail-nav">
            {visibleMenus.map((menu) => (
              <Link key={menu.href} href={menu.href} className={active === menu.activeKey || pathname === menu.href ? "active" : ""}>
                {menu.label}
              </Link>
            ))}
